...
We executed a rigorous, pre-registered intervention to validate whether the structural head roles discovered in the atlas truly dictate model behavior. We built a native routing engine for Qwen2.5-0.5B that intercepts head outputs during the forward pass and forces them into highly constrained attention kernels.

### 1. The Local Head Success
For heads classified as **Local** and proving stable across 4 domains (Wikipedia, Code, Dialogue, Math), we constrained them to a strict **32-token sliding window**. This affected 130 heads (38% of the model).
...
**Hypothesis from Phase 1:** Sink heads are "dumb" waste-baskets. If a head isn't needed, it safely rests on the BOS token. Thus, restricting a Sink head should be safe and cheap.

**Result: Spectacularly Falsified.**

We built an Attention Routing Engine that intercepts the forward pass and forces heads to ONLY attend to their structurally predicted targets (e.g. Local heads can only see a 32-token sliding window; Sink heads can only see the BOS token).

- **Restricting Local Heads**: When we restricted Local heads to a 32-token window, HellaSwag accuracy barely degraded (from 60.1% to 58.4%).
- **Restricting Sink Heads**: When we forced Sink heads (which *already* spend >50% of their mass on the BOS token) to look *exclusively* at the BOS token, **accuracy catastrophically collapsed to 26% (random chance).**

This completely overturns our Workstream 2 hypothesis. Sink heads are **not** resting. They are performing critical global context aggregation. When you force them into a naive "cheap BOS routing" state, the model is lobotomized.
