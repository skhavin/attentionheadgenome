...
def evaluate_ruler(model, tokenizer, head_classes):
    ...
    def run_niah(use_router=False):
        ...
        if use_router:
            ...
            def get_hook(layer_idx):
                def pre_hook(module, args, kwargs):
                    ...
                    if q_len != seq_len: return args, kwargs # Only apply mask during prefill, generation is unmasked (or use_cache=False handles it)
        ...
        with torch.no_grad():
            # Setting use_cache=False prevents KV cache shape mismatches in SDPA
            outputs = model.generate(**inputs, max_new_tokens=15, pad_token_id=tokenizer.eos_token_id, use_cache=False)
            
        for h in hooks: h.remove()
        
        generated = tokenizer.decode(outputs[0][seq_len:], skip_special_tokens=True)
        return "TritonIsFast42" in generated, generated

    return run_niah(False), run_niah(True)

def main():
    print("="*60)
    print("  PHASE 1 WRAP-UP: RULER (NIAH) BENCHMARK")
    print("="*60)
    ...
    for model_id in MODELS:
        ...
        try:
            ...
            (base_pass, base_out), (rout_pass, rout_out) = evaluate_ruler(model, tokenizer, head_classes)
            print(f"      Baseline Acc: {base_pass} | Router Acc: {rout_pass}")
            print(f"      (Base Output: {base_out.strip()})")
            print(f"      (Rout Output: {rout_out.strip()})")
