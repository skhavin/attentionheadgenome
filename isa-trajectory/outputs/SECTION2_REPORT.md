...
*   **The Structural Climb & Probe Correlation**: The geometric divergence curves strongly track the monotonic emergence of the linear probes for models that reach saturation. However, the correlation is heavily disrupted by the post-peak decline (see below). When we isolate the correlation to the **pre-peak (climb) region**, the relationship becomes nearly perfect across all three architectures:
    *   **Qwen2.5-1.5B**: Pre-peak $r = 0.933$ (Full curve $r = 0.793$). Divergence climbs smoothly from L0 to peak around L21.
    *   **Phi-1.5**: Pre-peak $r = 0.961$ (Full curve $r = 0.892$). Climbs continuously to peak at L14.
    *   **Llama-3.2-1B**: Pre-peak $r = 0.863$ (Full curve $r = 0.323$). Llama's rapid F-ratio spike perfectly correlates with its early probe emergence. The weak full-curve correlation is entirely due to Llama entering its post-peak decline phase far earlier than the other models (at Layer 9 of 16).

*   **The Post-Peak Decline (Lexical Scattering)**: Across all three models, the F-ratio declines after reaching its peak. This initially seemed to contradict a "developmental tree," implying branches might be merging back together. We empirically tested this by measuring raw distances at Qwen's peak (L21) vs final layer (L27). We found the categories do *not* compress together: the Between-Category variance (SSB) actually exploded by 5x (89k → 481k). However, the Within-Category variance (SSW) exploded even faster, growing nearly 7x (331k → 2.1M). 
    *   **Conclusion**: The unembedding layers do not funnel operations together; they **lexically scatter** them. Once the cognitive operation (e.g., "Arithmetic") is fully constructed at the F-ratio peak, the final layers must shatter that operational manifold into individual, orthogonal vocabulary token predictions (e.g., "15" vs "2"). The categories themselves remain far apart, but the internal points scatter so violently that the *relative* clustering (F-ratio) degrades.

**Conclusion**: The separation of cognitive operations is not a rapid, single-layer state injection. It is an organized geometric expansion—a developmental tree where operations gradually branch away from each other across network depth, before ultimately undergoing lexical scattering at the unembedding boundary.
