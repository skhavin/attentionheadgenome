# phase1/step10_retrieval_entropy_qwen15b.py
#
# PURPOSE:
#   Entropy-collapse experiment on Qwen2.5-1.5B (GQA group=6).
#   Extends Qwen-0.5B analysis with:
#     (a) KV group sink clustering analysis — tests whether layer-9/16 sink
#         clustering in 0.5B is KV-group-driven or layer-depth-driven
#     (b) Cross-model comparison of retrieval head abundance and depth
#
# ARCHITECTURE:
#   28 layers, 12 Q heads, 2 KV heads, GQA group=6
#   (vs Qwen-0.5B: 24L, 14Q, 2KV, group=7)
#   Less extreme KV sharing → expect more retrieval specialization if
#   the "KV sharing suppresses retrieval" hypothesis holds.
#
# OUTPUTS:
#   outputs/phase1/qwen1.5b_retrieval_entropy.json
...
def attention_entropy(attn_weights):
    ...
    # Guard against NaN rows (can appear in Qwen-1.5B bf16 on very short seqs):
    # replace NaN rows with uniform distribution so entropy = log(seq_len).
    last_pos = attn_weights[:, -1, :].float()   # (num_q_heads, seq_len)
    # Replace any NaN values with zero, then renormalise
...
def analyze_sink_clustering(sink_heads_by_layer, gqa_group, num_layers):
    """
    For each layer with multiple sink heads, measure:
    - How many KV groups are represented
    - Whether clustering is KV-group-driven vs layer-driven
    """
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.bfloat16,   # bfloat16 avoids fp16 overflow NaN in Qwen-1.5B
    )
    if device == "cuda":
        model = model.cuda()
    model.eval()
    ...
    num_q_heads  = model.config.num_attention_heads  # 12
    num_kv_heads = model.config.num_key_value_heads  # 2
    gqa_group    = num_q_heads // num_kv_heads       # 6
    ...
    # De-duplicate sink_heads_by_layer (NaN heads added twice above)
    for l in sink_heads_by_layer:
        sink_heads_by_layer[l] = sorted(set(sink_heads_by_layer[l]))

    # ── Print results ─────────────────────────────────────────────────────────
    total = sum(counts.values())
    print("\n=== Qwen2.5-1.5B Mechanistic Labels ===")
    ...
    print("\n=== Cross-Model Comparison (Qwen family) ===")
    print("  Model        GQA_grp  total_heads  sink%   ret%    ind%")
    print("  Qwen-0.5B    7        336          10.7%   1.2%    5.4%")
    print("  Qwen-1.5B    6        " + str(num_layers * num_q_heads) +
          "      " +
          str(round((counts["sink"] / total) * 100, 1)) + "%    " +
          str(round((counts["retrieval"] / total) * 100, 1)) + "%     " +
          str(round((counts["induction"] / total) * 100, 1)) + "%")
    ...
    out_path = os.path.join(OUT_DIR, "qwen1.5b_retrieval_entropy.json")
